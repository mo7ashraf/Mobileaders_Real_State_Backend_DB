...
async function main() {
  ...
  for (const s of sellers) {
    ...
    await prisma.sellerProfile.upsert({ where: { userId: u.id }, update: { verified: !!s.verified, clients: Number(s.clients||0), rating: Number(s.rating||0), badges: JSON.stringify(s.badges||[]), joinedHijri: s.joinedHijri, joinedText: s.joinedText, regionText: s.regionText }, create: { userId: u.id, verified: !!s.verified, clients: Number(s.clients||0), rating: Number(s.rating||0), badges: JSON.stringify(s.badges||[]), joinedHijri: s.joinedHijri, joinedText: s.joinedText, regionText: s.regionText } });
  }
